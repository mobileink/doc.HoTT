...
For example, the codefinitions of \codefn{evens} and \ifdef{odds} over
...
only that it produces an \(X\) from a token of type \(\Colist{X}\).
...
\textit{how} it should do it.

The role of cotypes codefined by coinduction is to serve as the
codomain for cofunctions. Cofunctions are codefined by specifying the
``action'' of the co-constructors on the result of using the
