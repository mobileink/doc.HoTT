...
of a function, we write \(f\defn\lambda x.M\). This means: ``\(f\)
...
Then we would have \(T\defn \tau.\textsf{expr}\), meaning ``\(T\)
names the type determined by the tau expression
\(\tau.\textsf{expr}\)''.  Let's try this with \(\Nat\):
\[\Nat\defn\tau.\tj{\Znat}{\Nat}, \tj{n}{\Nat}\linfer\tj{Sn}{\Nat}\]
...
name:
\[\Nat\defn\tau.\tj{\Znat}{\tau}, \tj{n}{\tau}\linfer\tj{Sn}{\tau}\]
...
\begin{align}
  ...
  \tj{x}{X}, \tj{xs}{\List\,X} &\linfer \tj{x\cons{}xs}{\List\,X} \\
\end{align}
...
\begin{align}
  \tj{xs}{\List\,X} & \linfer \tj{\head(xs)}{X} \\
  \tj{xs}{\List\,X} & \linfer \tj{\tail(xs)}{\List X}
\end{align}

\begin{align}
  \tj{x}{X},\tj{xs}{\List\,X} & \linfer \tj{\tail(x\cons{}xs)} = xs \\
  \tj{x}{X},\tj{xs}{\List\,X} & \linfer \tj{\head(x\cons{}xs)} = x
\end{align}
...
e
\begin{align}
  \tj{\iseq{xs}}{\Stream\,X} & \linfer \tj{\head(\iseq{xs})}{X} \\
  \tj{\iseq{xs}}{\Stream\,X} & \linfer \tj{\tail(\iseq{xs})}{\Stream\,X}
\end{align}
...
For example, the codefinitions of \ifndef{even} and \ifdef{odd} over
...
In other words, the codefinition of the coconstructor \ihead tells us
only that it produces an \(X\) from a token of type \(\Stream\, X\).

Here's a codefinition of the coconstructors for type \(\Stream\, X\):

\begin{align}
  \ihead(\ifndef{even}(\iseq{xs})) & = \ihead(\iseq{xs}) \\
  \itail(\ifndef{even}(\iseq{xs})) & = \itail(\itail(\iseq{xs})) \\
  \ihead(\ifndef{odd}(\iseq{xs})) & = \ihead(\itail(\iseq{xs})) \\
  \itail(\ifndef{odd}(\iseq{xs})) & = \itail(\itail(\itail(\iseq{xs})))
\end{align}

\subsubsection{Sequent Calculus for Cologic}
...
\begin{prooftree}
\AxiomC{$\Gamma\linfer \tj{\iseq{xs}}{\Stream\,X}$}
\AxiomC{$\Gamma,\tj{b}{B}\linfer \tj{c}{C}$}
\BinaryInfC{$\Gamma,\head(xs)\linfer \tj{c}{C}$}
\end{prooftree}

This does not really work. It expresses both introduction and
elimination of \head.
...
arbitrary elements of the type. For any \(\tj{xs}{\Stream X}\) we have
\(\tj{\head(xs)}{X}\) and \(\tj{\tail(xs)}{\Stream X}\).
...
For example, we can define \ifndef{even} and \ifndef{odd} over type
\(\Stream\, X\):

\begin{align}
  \iseq{xs} & \defn [xs_0, xs_1, \ldots] \\
  \ifndef{even}(\iseq{xs}) & = [xs_0, xs_2, \ldots] \\
  \ifndef{odd}(\iseq{xs})  & = [xs_1, xs_3, \ldots]
\end{align}
...
construct them. For a finite list, we would define \fndef{even} and
\fndef{odd} by pattern-matching against the internal structure of the
argument, expressed by constructor patterns starting with the base case:

\begin{align}
  \fndef{even}([]) & = [] \\
  \fndef{odd}([]) & = [] \\
  \fndef{even}(x::xs) & = x::\fndef{even}(\tail(xs)) \\
  \fndef{odd}(x::xs) & = \head(xs)::\fndef{odd}(\tail(xs))
\end{align}
...
Now back to greatest fixed point.
...
\begin{displayquote}[\cite{hottbook} p.48; emph. added]
  ...
  equal {\upshape [definitionally bound]}, \(a\defn b\), then we also
...
%%           {a\defn b\type A}
%%           %% {\Gamma,a\defn b\type A \linfer a\type Q}
...
\section{Codefinition \& Coequality}

Treat \(\defn\) as codefinition?
